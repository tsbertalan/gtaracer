...
class RobustSpline:
    ...
    def __init__(self, estimator='RANSAC', n_knots=30, **estimator_kwargs):
    ...
    def fit(self, X, y):
        ...
        # Construct the core estimator.
        if self.estimator_kind == 'RANSAC':
            self.estimator_kwargs.setdefault('random_state', 42)
            self.estimator = RANSACRegressor(**self.estimator_kwargs)
        elif self.estimator_kind == 'TheilSen':
            self.estimator = TheilSenRegressor(**self.estimator_kwargs)
        elif self.estimator_kind == 'Huber':
            self.estimator = HuberRegressor(**self.estimator_kwargs)
        else:
            self.estimator = LinearRegression(**self.estimator_kwargs)
        
        # Construct the pipeline.
        self.pipeline = make_pipeline(bspline_features, self.estimator)
    ...
    @property
    def inliers(self):
        assert self.estimator_kind == 'RANSAC'
        return self.estimator.inlier_mask_
